<%@ Page Title="Master Type Activity Group" Language="vb" AutoEventWireup="false" MasterPageFile="~/Site.Master" CodeBehind="Masteractivitygroup.aspx.vb" Inherits="IME.Masteractivitygroup" %>
                        ...
                        <div class="row vert-pad-10px">
                            <div class="col-md-3">
                                <div class="form-group btn-group">
                                    <a href="./Masteractivitygroup-data.aspx" class="btn btn-default"><i class="fa fa-plus "></i></a>
                            ...
                            <div class="col-md-3">
                                <input type="text" class="form-control inline-block w75" id="searchby" data-ng-model="searchInput.id" />
                            ...
                            <table class="table table-bordered">
                                <thead>
                                    <tr>
                                        <th>Action</th>
                                        <th>Id</th>
                                        <th>Kode Jenis Komoditi</th>
                                        <th>Grup Aktifitas</th>
                                        <th>Berlaku Mulai</th>
                                ...
                                <tbody>
                                    <tr data-ng-repeat="item in items">
                                        <td>
                                            <input type="checkbox" />
                                            <a href="Masteractivitygroup-data.aspx?id={{ item.mastertypeaktif_id }}"><i class="fa fa-edit" ></i></a>
                                        </td>
                                        <td>{{ item.mastertypeaktif_id }}</td>
                                        <td>{{ item.jenis_komodity }}</td>
                                        <td>{{ item.grup_aktifitas }}</td>
    ...
    <script type="text/javascript">
        ...
        var pagecontrol = function ($scope, $filter, $window, $http) {
            ...
            $scope.initializedata = function () {
                var post = $http({
                    method: "POST",
                    url: "./Masteractivitygroup.aspx/InitializeData",
                    dataType: 'json',
                    data: { },
            ...
            $scope.searchid = function () {
                $scope.searchInput.start_date = moment(vm.dateFrom._d);

                var post = $http({
                    method: "POST",
                    url: "./Masteractivitygroup.aspx/SearchData",
                    dataType: 'json',
                    data: { input: $scope.searchInput },
                    headers: { "Content-Type": "application/json" }
                }).then(function successCallBack(response) {
                    $scope.items = response.data.d;
                    console.log($scope.items);
